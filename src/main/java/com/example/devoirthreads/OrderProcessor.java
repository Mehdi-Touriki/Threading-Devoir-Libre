    ...
    public void run() {
        synchronized (mutex) {
            try {
                ...
                ObjectNode rootNode = (ObjectNode) mapper.readTree(new File("Files/input.json"));
                ...
                mapper.writerWithDefaultPrettyPrinter().writeValue(new File("Files/output.json"), outputRoot);

                ObjectNode errorRoot = mapper.createObjectNode();
                errorRoot.set("orders", mapper.valueToTree(invalidOrders));
                mapper.writerWithDefaultPrettyPrinter().writeValue(new File("Files/error.json"), errorRoot);
